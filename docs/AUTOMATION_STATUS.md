...
| Discord-1a incoming webhook notification-only | `DESIGNED_ONLY` | 알림 전용 단계의 구현 전 계약 문서만 있습니다. Webhook sender는 없습니다. |
| Discord-1b/1c/2a/2b/3a/3b/3c/4a/4b/5 rollout | `DESIGNED_ONLY` | rollout 단계만 문서화되어 있습니다. |
...
| Discord command bot implementation | `PROHIBITED` | Discord-1a 구현 전에는 slash command나 command bot을 만들지 않습니다. |
...
1. `docs/AUTOMATION_STATUS.md`를 merge합니다.
2. PR #37 이후 별도 새 learning note가 없음을 확인합니다.
3. AI Review Gate coverage gap 개선 계획을 세웁니다.
   - Documentation Rules: Korean prose + English stable terms
   - placeholder / `OWNER_DECISION_REQUIRED` rule
   - merged PR `@codex review` loop handling
4. Codex Review integration은 바로 required gate가 아니라 report-only/shadow-review로 검토합니다.
5. Discord-1a notification-only 구현 전 owner decisions를 확정합니다.
6. 그 뒤 Discord-1a implementation planning으로 넘어갑니다.
...
- Discord-1a implementation PR의 allowed files
